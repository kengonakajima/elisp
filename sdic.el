...
;;; $Id: sdic.el,v 2.6 1999/01/31 11:52:56 tsuchiya Exp $
...
;;     /usr/dict/gene.sdic を sdic-grep.el を使って検索する場合は次の
;;     ようになります。
;;
;;         (setq sdic-eiwa-dictionary-list '((sdic-grep "/usr/dict/gene.sdic")))
;;
;;     複数の辞書を同時に検索することも出来ます。
;;
;;         (setq sdic-waei-dictionary-list '((sdic-array "~/data/jedict.sdic")
...
(defvar sdic-eiwa-dictionary-list
  (let ((file "/home/ringo/lisp/gene.sdic"))
...
(defvar sdic-waei-dictionary-list
  (let ((file "/home/ringo/lisp/jedict.sdic"))
    (and (file-readable-p (expand-file-name file))
	 (list (list (if (string-match "\\.sdic$" file)
			 (if (and (require 'sdic-grep)
				  (sdic-grep-available-p))
...
(defun sdic-read-from-minibuffer (&optional init pre-prompt)
  ...
  (let ((w (or init (sdic-word-at-point) "")))
    (setq sdic-read-minibuffer-history (cons w sdic-read-minibuffer-history)
	  w (downcase (read-from-minibuffer (if pre-prompt
						(format "%s Input word : " pre-prompt)
					      "Input word : ")
					    w nil nil '(sdic-read-minibuffer-history . 1))))
    (if (>= (length w) 2) w
      (downcase (read-from-minibuffer (format "\"%s\" is too short. Input word again : " w)
				      w nil nil '(sdic-read-minibuffer-history . 1))))))
...
検索結果表示バッファを削除する関数"
  (interactive)
  (if (sdic-buffer-live-p (get-buffer sdic-buffer-name))
      (progn
	(sdic-close-window)
